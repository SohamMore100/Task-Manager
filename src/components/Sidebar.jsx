...
const Sidebar = ({
  ...
  const todaysTasks = tasks.filter(
    (task) => moment(task.dueDate).format("DD/MM/YYYY") === today
  );
  ...
  return (
    <aside className="w-1/4 bg-white-100 dark:bg-slate-900">
      <h2 className="pt-8 text-center font-bold text-2xl text-gray-600 dark:text-gray-200">
        TASK MANAGER
      </h2>
          ...
          <li
            ...
            className={`relative cursor-pointer py-2 pl-4 ${
              activeCategory === category
                ? "bg-purple-100 text-red-500 dark:bg-purple-100 dark:text-red-500"
                : ""
            }`}
      ...
      {/* Internship interview live task */}
      {/* <button
      type="button"
        onClick={() => {
  ...
  className="bg-red-500 text-white hover:bg-red-700 px-8 py-2 rounded-lg shadow-md text-lg dark:bg-zinc-100 dark:hover:bg-zinc-200 dark:text-red-500"
>
  <span>Add task</span>
</button> */}
